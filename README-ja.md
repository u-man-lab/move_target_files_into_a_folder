...
1. 最初のスクリプト[`move_target_files_into_a_folder.py`](#1-move_target_files_into_a_folderpy)は、TXTファイルにリストアップされた複数のファイルを単一のフォルダに統合し、元の絶対パスをファイル名にエンコードします。設定ファイルへのオプション指定により、移動ではなくコピーを実行させ、元のファイルを残すこともできます。
